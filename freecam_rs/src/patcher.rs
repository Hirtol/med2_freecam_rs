...
impl LocalPatcher {
    ...
    ///
    /// `local_ptr` must be valid within the current memory space.
    pub unsafe fn safe_read_slice<'a, 'b, T>(&'a self, local_ptr: *const T, length: usize) -> &'b [u8] {
    ...
    pub unsafe fn unpatch(&mut self, local_ptr: *mut u8) {
        if let Some(index) = self.patches.iter().rev().position(|value| value.address == local_ptr) {
